...
public class LSystemsB : MonoBehaviour {
	...
	void SetupCone(){
		...
		Vector3 iniPos = new Vector3(segLength, 0f,0f);
		
	//	float tRand = Random.Range(-1f,1f)
		Vector3 rotVect = new Vector3(1f,0f,0f);
		//+(new Vector3(Random.Range(-1,1f),0f,Random.Range(-1,1f))).normalized;
		
		segmentPos.Add(new Vector3(1f,0f,0f));
		
		
		segmentRot.Add(Quaternion.AngleAxis( 0.5f*iniAngle, rotVect));
		//segmentRot.Add(Quaternion.AngleAxis( 0f, rotVect));
		...
		for(int j=1;j<numberSegments[currentBranchId];j++){
		    ...
		    segmentRot[currentBranchId] = Quaternion.AngleAxis( angle[currentBranchId], rotVect+(new Vector3(Random.Range(-0.4f,0.4f)*rotVect.x,Random.Range(-0.4f,0.4f)*rotVect.y,Random.Range(-0.4f,0.4f)*rotVect.z)) );
		...
		//    rotVect = rotVect+(new Vector3(Random.Range(-1.4f,1.4f)*rotVect.x,Random.Range(-1.4f,1.4f)*rotVect.y,Random.Range(-1.4f,1.4f)*rotVect.z));
         //   rotVect = (new Vector3(Random.Range(-1,1f),0f,Random.Range(-1,1f))).normalized;
			...
			iniAngle = -10f/segLength*((iniAngle+0.001f)/Mathf.Abs((iniAngle+0.001f)));
